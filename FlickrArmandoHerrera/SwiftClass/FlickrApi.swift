...
class FlickrApi: ObservableObject {
    ...
    func getUrl(searchText: String, page: Int = 1, perpage: Int = 18) -> String {
        ...
        let safeLongTxt = _cat.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)!
        
        let safeUrl = "https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=f46b7b3920dc5855bdfdbeacd14b3ebd&tags=\(safeLongTxt)&text=\(safeText)&privacy_filter=1&content_type=1&extras=url_s%2C+url_o%2C+url_t%2C+url_m%2C+url_l&per_page=\(perpage)&page=\(page)&format=json&nojsoncallback=1"
